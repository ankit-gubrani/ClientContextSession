if (CQ_Analytics.MyContextStore) {
    ...
    CQ_Analytics.MyContextStore.createHTMLElement = function () {
        ...
        if (nbProp == 0) {
            var li = document.createElement("div");
            li.className = "marketoentry noprop";
            li.innerHTML = "No Marketo Lead Information";
            div.appendChild(li);
        }
    ...
    CQ_Analytics.MyContextStore.renderer = function (store, targetId) {
        CQ_Analytics.MyContextStore.targetId = targetId;
        if (store && store.STORENAME == CQ_Analytics.MyContextStore.STORENAME) {
